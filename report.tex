...
% ---- TikZ for architecture diagram + Gantt chart ---------------------
\usepackage{tikz}
...
\begin{description}[leftmargin=1.4cm,style=nextline]
    ...
    \item[Passive Gravity-Sorting Arrays.] Low-maintenance internal
    routing relying on momentum, Teflon-coated surfaces, and simple
    pneumatic diverters rather than per-floor robot arms.
\end{description}
...
\begin{tikzpicture}[
    ...
    layer/.style={rectangle, rounded corners, draw, thick,
                  text width=3.6cm, align=center, minimum height=1.4cm,
                  fill=blue!5},
    edge/.style={-{Latex[length=2.5mm]}, thick}
]
\node[layer] (cloud) {\textbf{Central Logistics Cloud}\\(The ``Brain'')\\Fleet API Gateway, BAS Controller, Security \& Auth};
\node[layer, below=of cloud] (edge) {\textbf{External Intake Nodes}\\(The ``Edge'')\\Aerial rooftop node, Ground micro-vehicle node};
\node[layer, below=of edge] (spine) {\textbf{Vertical Transport Core}\\(The ``Spine'')\\New-build conveyors \& diverters; Retrofit tube \& claws};
\node[layer, below=of spine] (last)  {\textbf{Delivery Endpoints}\\(The ``Last Meter'')\\Hallway smart lockers; Retrofitted window-box lockers};
\node[layer, below=of last]  (app)   {\textbf{End-User Interface}\\(The App)\\Resident notifications and authentication};

\draw[edge] (cloud) -- (edge);
\draw[edge] (edge) -- (spine);
\draw[edge] (spine) -- (last);
\draw[edge] (last) -- (app);
\draw[edge, dashed] (app.east) to[bend left=60] (cloud.east);
\end{tikzpicture}
